            ...
            if self.access1 in channelValue:
                if not deleted:
                    ...
                    if asyncio.iscoroutinefunction(self.callback):
                        ...
                        if "var_A" in data_name or "var_S" in data_name:
                            self.loop.create_task(async_send_to_pipe_channel(
                                channel_name="plotly_basic",
                                label="spec_1D",
                                value={data_name: channelValue,
                                       "epoch": epoch,
                                       }
                            ))
...
class SpecProtocol(asyncio.Protocol):
    ...
    def data_received(self, data):
        asyncio.async(self.readFromStream(data))
...
class SpecClient:
    def __init__(self, host, port):
        ...
        self.send_command("p \"SpecClient v0.1, Connected\"")
    ...
    def map_holder(self): 
        self.send_command("epics_put(\"SMTR16114I1030:power\",1); epics_put(\"SMTR16114I1031:power\",1);zero_xy_stage; cmesh xp -5.99 4.99 3 yp -4.9 5 50; epics_put(\"SMTR16114I1031:power\",2); epics_put(\"SMTR16114I1030:power\",2)")
        ...
        with open("/home/sgm/SpecMacros/plate.mac", 'w') as f:
            for n, s in enumerate(plate):
                ...
                for i, sample in enumerate(s['sample']):
                        f.write("sample \"%s\" %d \n" % (sample, s['type']))
                        f.write("epics_put(\"SMTR16114I1030:power\",1); epics_put(\"SMTR16114I1031:power\",1) \n")
                        f.write("zero_xy_stage \n")
                        coord = s['coords'][i]
                        f.write("umv xp %f; umv yp %f \n" % coord)
                        f.write("epics_put(\"SMTR16114I1030:power\",2); epics_put(\"SMTR16114I1031:power\",2) \n")
                        if 'scan' in s.keys():
                            scan = s['scan']
